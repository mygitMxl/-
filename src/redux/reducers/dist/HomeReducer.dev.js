...
var HomeReducer = function HomeReducer() {
  ...
  switch (action.type) {
    ...
    case 'change_hone_data':
      return state.merge({
        topicList: (0, _immutable.fromJS)(action.topicList),

        /* 不转成immutable组件就无法接 */
        articleList: (0, _immutable.fromJS)(action.articleList),
        recommendList: (0, _immutable.fromJS)(action.recommendList)
      });
    ...
    case 'addList':
      return state.merge({
        ...
        /* 注意获取之前的state,用到是state */

        /* concat 数组的API 连接两个数组 */
